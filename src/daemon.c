...
int draw_image_on_output(struct output *o, const char *image_path,
  ...
  case IMAGE_MODE_CENTER:
    ...
    if (!resized) {
      _log("ERROR", "Failed to resize %s from %dx%d to %dx%d", image_path, iw,
           ih, o->width, o->height);
  ...
  case IMAGE_MODE_FIT:
    ...
    break;
  default:
    _log("ERROR", "Unknown mode: %c", mode);
    return 1;
  }
  ...
  attach_and_commit();
  _log("INFO", "Set %s on %s. Mode: %c", image_path, o->name, mode);
...
int main(int argc, char **argv) {
  enum image_modes mode = 'C';
  const char *apath = "/media/linux/pictures/yurinight.jpg";

  xdwl_list *outputs = init();
  if (!outputs) {
    _log("ERROR", "Failed to init");
    _abort(errno);
  }
  ...
  for (; outputs->next; outputs = outputs->next) {
    ...
    int ret = draw_image_on_output(o, apath, mode);
    if (ret != 0)
      _abort(1);
  }
